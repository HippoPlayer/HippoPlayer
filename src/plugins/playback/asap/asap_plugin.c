...
enum HippoProbeResult asap_probe_can_play(const uint8_t* data, uint32_t data_size, const char* filename, uint64_t total_size) {
    ASAP* song = ASAP_New();
...
static int asap_read_data(void* user_data, void* dest, uint32_t samples_to_read) {
	struct ReplayerData* data = (struct ReplayerData*)user_data;
	...
	int gen_samples = ASAP_Generate(data->song, (uint8_t*)buffer, sizeof(buffer), ASAPSampleFormat_S16_L_E) / 2;

	const float scale = 1.0f / 32767.0f;
	float* output = (float*)dest;

	if (ASAPInfo_GetChannels(ASAP_GetInfo(data->song)) == 2) {
...
static int asap_metadata(const char* filename, const HippoServiceAPI* service_api) {
	...
	if (ext_offset != 0) {
    	HippoMetadata_set_tag(metadata_api, index, HippoMetadata_SongTypeTag, ASAPInfo_GetExtDescription(extension + 1));
	} else {
