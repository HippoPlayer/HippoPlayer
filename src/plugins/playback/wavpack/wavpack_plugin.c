...
static int wavpack_open(void* user_data, const char* filename, int subsong) {
    ...
    hp_debug("Open wavpack file with sample rate %d channels %d bytes per sample %d\n", data->sample_rate,
             data->channel_count, data->bytes_per_sample);
...
static HippoReadInfo wavpack_read_data(void* user_data, void* dest, uint32_t max_output_bytes,
    ...
    HippoReadInfo t = {data->sample_rate, samples_to_read * 2, data->channel_count, output_mode};
...
static int wavpack_metadata(const char* filename, const HippoServiceAPI* service_api) {
    ...
    static IdMap s_ids[] = {
        {"Title", HippoMetadata_TitleTag},     {"Album", HippoMetadata_AlbumTag}, {"Artist", HippoMetadata_ArtistTag},
        {"Comment", HippoMetadata_MessageTag}, {"Genre", HippoMetadata_GenreTag}, {"Year", HippoMetadata_DateTag},
    };
