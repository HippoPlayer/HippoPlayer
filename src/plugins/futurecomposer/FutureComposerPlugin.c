...
static struct FCReplayerData g_replayerData;
...
static int fcReadData(void* userData, HippoPlaybackBuffer* dest)
{
	struct FCReplayerData* replayerData = (struct FCReplayerData*)userData;	
    fc14dec_buffer_fill(replayerData->decoder, dest->data, 16 * 1024);
	return 0;
}
