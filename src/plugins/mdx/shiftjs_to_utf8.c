...
char* sj2utf8(const char* input)
{
	...
	char* output = malloc(len * 6);
	memset(output, 0, len * 6);
    ...
    while (indexin < len)
    {
        ...
        uint16_t value = (((uint16_t)convTable[offset]) << 8) | convTable[offset + 1];
        if(value < 0x80)
        {
