...
#define HSIntValue(id, name, desc, value) { .int_value = { HS_INTEGER_TYPE, id, name, desc, value, -1, ~0 } }
#define HSFloatValue(id, name, desc, value) { .float_value = { HS_FLOAT_TYPE, id, name, desc, value, 0.0f, 10.0f } }
#define HSBoolValue(id, name, desc, value) { .bool_value = { HS_BOOL_TYPE, id, name, desc, value } }
#define HSIntValue_Range(id, name, desc, value, min, max) { .int_value = { HS_INTEGER_TYPE, id, name, desc, value, min, max } }
#define HSIntValue_DescRange(id, name, desc, value, ranges) { .int_fixed_value = { HS_INTEGER_RANGE_TYPE, id, name, desc, value, (HSIntegerRangeValue*)&ranges, hp_sizeof_array(ranges) } }
#define HSStringValue_DescRange(id, name, desc, value, ranges) { .string_fixed_value = { HS_STRING_RANGE_TYPE, id, name, desc, value, (HSStringRangeValue*)&ranges, hp_sizeof_array(ranges) } }
...
typedef struct HippoSettingsAPI {
    ...
    HippoSettingsError (*register_filetype_settings)(void* priv_data, const HSSetting* settings, int count);
    HippoSettingsError (*register_global_settings)(void* priv_data, const HSSetting* settings, int count);
...
#define HippoSettings_register_filetype_settings(api, settings) \
    api->register_filetype_settings(api->priv_data, (HSSetting*)&settings, hp_sizeof_array(settings))

#define HippoSettings_register_global_settings(api, settings) \
    api->register_global_settings(api->priv_data, (HSSetting*)&settings, hp_sizeof_array(settings))

#define HippoSettings_get_last_error(api) \
    api->get_last_error(api->priv_data)
